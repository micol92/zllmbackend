...
async function insertMessage(messageEntity, messageRecord, conversationId, conversationEntity, messageTime) {
   ...
   const existingConversation = await SELECT.one.from(conversationEntity).where({ cID: conversationId });

    if (!existingConversation) {
        console.log("Conversation not found, creating new one for:", conversationId);
        await INSERT.into(conversationEntity).entries([{ cID: conversationId, last_update_time: messageTime }]);
    } else {
...
async function handleMemoryAfterRagCall(conversationId, message_time, chatRagResponse, Message, Conversation, fileName) {
    try {
